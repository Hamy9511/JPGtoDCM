...
#Titulo
fInput =customtkinter.CTkFrame(window, bg="gray", padx=5, pady=5, borderwidth=5, relief="sunken") #Creación de un cajon
fInput.grid(row=0, column=0, rowspan=1, columnspan=2, sticky=tk.NSEW, padx=20, pady=20,) #Asignar y configurar celda del grid
lInput = customtkinter.CTkLabel(master=fInput, text="INGRESAR DATOS DEL PACIENTE", font=("arial", 14, "bold italic"), bg="gray") #Label asignado al frame
#lInput.config(font=("arial", 14, "bold italic")) <-------------- Otra forma de agregar cambios al frame, label, etc.
lInput.pack() #Ejecutando el label

#ID
fId =customtkinter.CTkFrame(master=window)
fId.grid(row=1, column=0, padx=10, pady=10)
lId = customtkinter.CTkLabel(master=fId, text="ID del paciente:", font=("arial", 10))
lId.pack()

#Nombre
fName =customtkinter.CTkFrame(master=window)
fName.grid(row=2, column=0, padx=10, pady=10)
lName = customtkinter.CTkLabel(master=fName, text="Nombre:", font=("arial", 10))
lName.pack()

#Apellido
fLastname =customtkinter.CTkFrame(master=window)
fLastname.grid(row=3, column=0, padx=10, pady=10)
lLastname = customtkinter.CTkLabel(master=fLastname, text="Apellido:", font=("arial", 10))
lLastname.pack()

#Fecha
fDate =customtkinter.CTkFrame(master=window)
